...
def doc_edit_form():
    with g.db.cursor() as c:
        ...
        if prev_content is None:
            new_document = True
            prev_content = ""
        tool.record_history(docid, int(new_document), value, None, None, tool.user(), request.form["edit_comment"], len(value) - len(prev_content))
...
def user():
    with g.db.cursor() as c:
        if tool.is_login():
            try:
                return tool.rt("user.html", user_name = tool.id_to_user_name(tool.user()), login=True)
            except:
                session.pop("id", None)
...
def delete(doc_name):
    i = tool.user(create = request.method == "POST")
    ns, name = tool.split_ns(doc_name)
    docid = tool.get_docid(ns, name)
...
def move(doc_name):
    ...
    if request.method == "POST":
        ...
        if "swap" in request.form:
            ...
            tool.record_history(docid, 3, tool.get_doc_data(docid), doc_name, to, tool.user(), request.form["note"], 0)
            tool.record_history(todocid, 3, tool.get_doc_data(todocid), to, doc_name, tool.user(), request.form["note"], 0)
        else:
            # 일반 문서 이동
    ...
    with g.db.cursor() as c:
        ...
        if request.method == "POST":
            ...
            if opcode == "add":
                ...
                if not nsacl: tool.record_history(docid, 4, tool.get_doc_data(docid), f'insert,{type2},{action},{"not:" if no else ""}{condtype}:{cond2 if condtype == "aclgroup" or condtype == "user" else cond}', None, tool.user(), "", 0)
            elif opcode == "delete":
                idx = json["index"]
                ...
                if f[0] == "user":
                    ...
                    v = f[1]
                if not nsacl: tool.record_history(docid, 4, tool.get_doc_data(docid), f'delete,{type2},{f[4]},{"not:" if f[3] else ""}{f[0]}:{v}', None, tool.user(), "", 0)
        ...
        if request.method == "POST":
            ...
            if request.form["mode"] == "ip":
                ...
                c.execute("INSERT INTO block_log (type, operator, target_ip, id, gid, date, duration, note) VALUES(1, ?, ?, ?, ?, ?, ?, ?)",
                        (tool.user(), ip, c.lastrowid, gid, t, dur, request.form["note"]))
            else:
                ...
                c.execute("INSERT INTO block_log (type, operator, target, id, gid, date, duration, note) VALUES(1, ?, (SELECT id FROM user WHERE name = ?), ?, ?, ?, ?, ?)",
                        (tool.user(), request.form["value"], c.lastrowid, gid, t, dur, request.form["note"]))"""
    ...
    with g.db.cursor() as c:
        if request.method == "POST":
            ...
            if len(logstr) != 0: c.execute("INSERT INTO block_log (type, operator, target, date, grant_perm, note) VALUES(3,?,?,?,?,?)",
                    (tool.user(), user, tool.get_utime(), " ".join(logstr), request.form["note"] if tool.get_config("ext_note") == "1" else ""))
            return '', 204
        else:
...
def discuss(doc):
    with g.db.cursor() as c:
        ...
        if request.method == "POST":
            ...
            slug = c.lastrowid
            #c.execute("INSERT INTO thread_comment (slug, no, text, type, author, time) VALUES(?,1,?,0,?,?)", (slug, request.form["content"], tool.user(), time))
...
#SELECT ?1, (SELECT seq FROM discuss WHERE slug = ?1), 0, ?2, ?3, ?4""", (slug, request.form["value"], tool.user(), tool.get_utime()))
...
def hide_thread_comment(slug, no, type):
    ...
    with g.db.cursor() as c:
        ...
        else: c.execute("UPDATE thread_comment SET blind = ?, blind_operator = ? WHERE slug = ? AND no = ?", (type, tool.user(), slug, no))
        return "", 204
...
def login_history():
    ...
    with g.db.cursor() as c:
        ...
        if "user" in request.args:
            ...
            c.execute("INSERT INTO block_log (type, operator, target, date, note) VALUES(4,?,?,?,?)", (tool.user(), id, tool.get_utime(), request.args["note"] if tool.get_config("ext_note") == "1" else ""))
...
def upload():
    if request.method == "POST":
        ...
        with g.db.cursor() as c:
            ...
            tool.record_history(docid, 1, content, None, None, tool.user(), request.form["note"], len(content))
...
def change_skin():
    ...
    skin = request.form["skin"]
    if skin == "":
        tool.del_user_config(tool.user(), "skin")
        return redirect(url_for("mypage"))
    if skin not in data.skins:
        return tool.error("invalid_skin")
    tool.set_user_config(tool.user(), "skin", skin)
    return redirect(url_for("mypage"))
...
def mark_troll_revision(doc):
    if not tool.has_perm("mark_troll_revision"):
        abort(403)
    with g.db.cursor() as c:
        c.execute("UPDATE history SET troll = ? WHERE doc_id = ? AND rev = ?", (tool.user(), tool.get_docid(*tool.split_ns(doc)), request.args.get("rev", type=int)))
    return "", 204
